...
const web3 = new Web3('https://rinkeby.infura.io/v3/api key from infura')
const account = '0x7D67e32C5d5A7e9a0F6E064913953Ac4B58F1f30'
web3.eth.accounts.wallet.add('your private key ') // Never use your real wallet here!
...
run = async()=>{
    //check Ether balance in our account
    let result 
    ...
    //check Dai balance
    //use contract.methods.balanceOf(account).call() to check the balance of ERC20 tokens
    ...
    console.log(`Successful tx: https://rinkeby.etherscan.io/tx/${result.transactionHash}`)
     ...
     //check Ether balance in our account after transfer
     result = await web3.eth.getBalance(account)
     result = web3.utils.fromWei(result,"Ether")
     console.log("Ether balance",result.toString())
 
     //check Dai balance once again after transfer
     //use contract.methods.balanceOf(account).call() to check the balance of ERC20 tokens
     result = await daiContract.methods.balanceOf(account).call()
     result = web3.utils.fromWei(result,'Ether')
     console.log('Dai Balance',result)
